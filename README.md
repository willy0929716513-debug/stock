...
2. **瀏覽器端練習帳戶**(`docs/assets/paper.js`)
   ...
   - 可跨日持倉,預設由使用者自己手動下單,也可以在頁面上開啟「🤖 自動交易模式」
     ...
     兩者不互斥。
   - **台股標的的收盤時間限制跟伺服器端帳戶一致**:台股(`category === "tw_stock"`)
     在台北時間 13:30 收盤後不會被自動模式下新單(買或賣都不會),美股/ETF/商品
     期貨/外匯/加密貨幣不受限制。跟伺服器端不同的是,這裡**不會**在收盤時強制
     平倉既有的台股持倉——這個帳戶本來就設計成可以跨日持倉,只有「開新單」要
     遵守收盤時間。
...
**誠實記錄實際踩到的問題**:一開始 `pipeline.yml` 排程設定 `*/5 * * * *`(每 5 分鐘),後來
又試過 `*/10`,但稽核真實執行紀錄後發現,GitHub Actions 的排程在公開 repo / 免費方案上對
這種高頻率排程並不可靠——實際觀察到的間隔是 **3~5 小時一次**,不是 5 或 10 分鐘,而且完全
沒有任何錯誤訊息,是 GitHub 平台本身把過於頻繁的排程丟進低優先權佇列、大量跳過,沒有 SLA
保證。單純調整 cron 數字沒辦法解決這個問題,因為問題根本不是「數字設多少」,是 GitHub
對高頻率排程本身的節流。

**現在的作法是繞過這個問題,不是跟 GitHub 的排程節流硬碰硬**:`pipeline.yml` 的 cron 改成
`0 * * * *`(整點觸發一次)——頻率低很多,GitHub 對這種頻率的排程可靠度好很多。真正的更新
頻率由**同一個 job 內部的迴圈**控制:一旦被觸發,就在同一個 job 裡每 5 分鐘跑一次
`daily_run.py` + `auto_trader.py` + commit,直到接近 GitHub-hosted runner 單一 job 6 小時的
硬性執行上限(跑滿 340 分鐘主動結束),把接力棒交給下一次整點觸發。`concurrency` 群組
確保重疊觸發只會排隊等目前這輪跑完,不會重複執行、也不會浪費資源。

需要澄清的一點:**抓取資料本身涵蓋全部資產類別、24 小時都在跑**,不會因為台股收盤
就縮小抓取範圍或暫停——排程本身沒有分台股/非台股。只有「要不要真的下單」這個決策
(`src/pipeline/auto_trader.py`、`docs/assets/paper.js` 的自動模式)才會依照台股交易時間
(9:00–13:30)決定台股標的能不能進場,美股/ETF/商品期貨/外匯/加密貨幣不受此限制。

另外加上一個獨立的保護機制(`src/pipeline/health_check.py`,有完整 pytest 測試):
每一輪迴圈之後,判斷「這次算不算真的抓到新資料」——訊號數量是 0(整批抓取失敗)、
或這次輸出跟上次 commit 完全一樣(可疑,真實市場報價幾乎不可能連續好幾次都毫無變動,
比較可能是抓取邏輯悄悄回傳了舊/快取資料而不是報錯)。連續 **3 次**符合以上任一種情況
(也就是最快約 15 分鐘內就能偵測到),就會呼叫 GitHub API **自動停用**這個 workflow,並在
`docs/data/pipeline_health.json` 留下記錄,避免系統已經壞掉了卻沒人發現、一直空轉。

**如果排程被自動停用了,要怎麼恢復**:先看 `pipeline_health.json` 跟對應那次 Actions run
的完整 log 找出真因(不要用猜的),修好之後到 repo 的 **Actions 分頁 → JPO-KBO Signal Pipeline →
